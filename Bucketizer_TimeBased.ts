...
export class TimeBucketizer extends BucketizerCore<TimeInputType> {
    ...
    private constructor(bucketizerOptions: TimeInputType, state?: any) {
        ...
        this.pageNumber = 0;
        this.lastTimestamp = new Date(Date.parse("2022-08-07T08:00:00Z"));
    ...
    public bucketizeNonChronological(quads: RDF.Quad[], memberId: string, minTimestamp: Date): RDF.Quad[] {
        ...
        if (quadTimestamp) {
            ...
            const bucketsToCreate = Math.ceil(timeDiff / this.period);
            if (timeDiff > 0) {
                for (let i = 0; i <= (bucketsToCreate + 1); i++) {
                    ...
                    const currentPage = this.pageNumber;
                    this.increasePageNumber();
                    const parameters = this.createRelationParameters(this.pageNumber, bucketStart, this.factory.namedNode('http://www.w3.org/ns/sosa/resultTime'));
                    this.setHypermediaControls(`${currentPage}`, parameters);
                    out.push(...this.expandRelation(`${currentPage}`, parameters));
                    const newBucket: timeBasedBucket = {
                        startTime: bucketStart, endTime: bucketEnd, pageNumber: currentPage
                    };
                    this.existingBuckets.push(newBucket);
                    this.increaseBucketNumber();
                }
                out.push(...this.createSDSRecord(this.factory.namedNode(memberId), [`${(this.pageNumber - 1)}`]));
                const newTimeStamp = new Date(minTimestamp.getTime() + (this.bucketCounter * this.period) + this.period);
                this.setLastTimestamp(newTimeStamp);
            } else {
